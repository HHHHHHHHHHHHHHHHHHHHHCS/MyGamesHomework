...
namespace Games_103_3
{
	public class SVD
	{
		...
		private float AccurateSqrt(float x)
		{
			return x / Mathf.Sqrt(x);
		}

		private void CondSwap(bool c, ref float X, ref float Y)
		{
			// used in step 2
			float Z = X;
			X = c ? Y : X;
		...
		private void CondNegSwap(bool c, ref float X, ref float Y)
		{
		...
		private void MultAB(float a11, float a12, float a13,
		...
		private void MultAtB(float a11, float a12, float a13,
			float a21, float a22, float a23,
			float a31, float a32, float a33,
			//
			float b11, float b12, float b13,
		...
		private void QuatToMat3(float[] qV,
			ref float m11, ref float m12, ref float m13,
			ref float m21, ref float m22, ref float m23,
			ref float m31, ref float m32, ref float m33
		)
		...
		private void ApproximateGivensQuaternion(float a11, float a12, float a22, ref float ch, ref float sh)
		{
			/*
		...
		private void JacobiConjugation(int x, int y, int z,
			ref float s11,
		...
		private float Dist2(float x, float y, float z)
		{
			return x * x + y * y + z * z;
		}

		// finds transformation that diagonalizes a symmetric matrix
		private void JacobiEigenanlysis( // symmetric matrix
			ref float s11,
			ref float s21, ref float s22,
			ref float s31, ref float s32, ref float s33,
		...
		private void SortSingularValues( // matrix that we want to decompose
		...
		private void QRGivensQuaternion(float a1, float a2, ref float ch, ref float sh)
		{
		...
		private void QRDecomposition( // matrix that we want to decompose
			float b11, float b12, float b13,
			float b21, float b22, float b23,
			float b31, float b32, float b33,
			// output Q
		...
		public void DoSVD(Matrix4x4 A, ref Matrix4x4 U, ref Matrix4x4 S, ref Matrix4x4 V)
		{
